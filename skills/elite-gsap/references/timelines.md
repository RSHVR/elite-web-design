...
master.add(createHeroAnimation()).add(createContentAnimation(), "-=0.5"); // Overlap by 0.5s
```
...
function createSectionReveal(section) {
  ...
  return tl
    ...
    .from(
      section.querySelectorAll(".section-item"),
      {
        opacity: 0,
        y: 30,
        stagger: 0.1,
...
function createModalAnimation(modal) {
  ...
  tl.to(modal, { autoAlpha: 1, duration: 0.3 }).from(
...
mm.add("(prefers-reduced-motion: no-preference)", () => {
  ...
  tl.from(".hero", { opacity: 0, y: 50, duration: 1 }).from(
    ".content",
